...
filtrer_data <- function(data) {
  filteredData <-
    matrix(ncol = 4)                                                      #matrice de 4 colonnes
  colnames(filteredData) <-
    c("date", "nombre_morts", "nombre_blesses", "etat")         #noms des colonnes
  for (row in 1:nrow(data)) {
    #parcours les données originales
    if (is.null(data[row, "date"]) |
        #si il manque une valeur on ignore cette ligne
    ...
    date <-
      data[row, "date"]                                                           #alors on remplie notre tableau avec les données
    ...
    aRow <- tryCatch({
      dateAsDate <-
        as.Date(date, "%Y-%m-%d", "%Y/%m/%d", FALSE)                        #convertie le format de la date
      if (is.na(dateAsDate))
        #si pas de date
...
data_set <-
  as.data.frame(data_set)                                                      #convertir en data frame (permet de stocker des éléments de types différents dans les mêmes colonne/lignes)
colnames(data_set) <-
  headers                                                            #rajoute les headers qui ont disparu lors de la conversion

donnees_filtrees <-
  filtrer_data(data_set)                                               #utilise la fonction précédement déclaré

liste_dates <-
  donnees_filtrees[, "date"]                                                #met les différents critères dans des listes
...
liste_morts.mean <-
  mean(liste_morts, na.rm = TRUE)                                      #moyenne et suppression de NA
liste_morts.sd <-
  sd(liste_morts, na.rm = TRUE)                                          #deviation standard
liste_morts.norm <-
  pnorm(liste_morts, m = liste_morts.mean, sd = liste_morts.sd)        #normes vectorielle
liste_morts.density <-
  density(liste_morts.norm, na.rm = TRUE)                           #estimation de densité
plot(liste_morts.density, main = "Fonction densité du nombre de morts")                  #trace la densité de morts

hist(
  #trace les historgrammes
  liste_morts,
  breaks = 50 ,
  xlim = c(0, 5),
...
nb_crimes_etats <-
  rle(sort(liste_etats))                                                #tri les etat par nombre de crimes 