...
barplot(vect_fusillade,
        names.arg = c("2013", "2014", "2015", "2016", "2017", "2018"))

# Fusillades réparties sur l'ensemble des jours
topdatearrange <-
  d %>% group_by(substr(d$date, 6, 10)) %>% summarize(n = n()) %>% arrange(-n)
topdatearrange

topdate <-
  d %>% group_by(substr(d$date, 6, 10)) %>% summarize(n = n())
topdate
barplot(topdate$n)

# Statistiques sur le 4 juillet (fête nationale), nombre de morts et blessés entre 2013 et 2017
blesses_13_07_04 <-
  sum(d %>% select(n_injured) %>% filter(d$date == "2013-07-04"))
blesses_14_07_04 <-
  sum(d %>% select(n_injured) %>% filter(d$date == "2014-07-04"))
blesses_15_07_04 <-
  sum(d %>% select(n_injured) %>% filter(d$date == "2015-07-04"))
blesses_16_07_04 <-
  sum(d %>% select(n_injured) %>% filter(d$date == "2016-07-04"))
blesses_17_07_04 <-
  sum(d %>% select(n_injured) %>% filter(d$date == "2017-07-04"))

morts_13_07_04 <-
  sum (d %>% select(n_killed) %>% filter(d$date == "2013-07-04"))
morts_14_07_04 <-
  sum(d %>% select(n_killed) %>% filter(d$date == "2014-07-04"))
morts_15_07_04 <-
  sum(d %>% select(n_killed) %>% filter(d$date == "2015-07-04"))
morts_16_07_04 <-
  sum(d %>% select(n_killed) %>% filter(d$date == "2016-07-04"))
morts_17_07_04 <-
  sum(d %>% select(n_killed) %>% filter(d$date == "2017-07-04"))
